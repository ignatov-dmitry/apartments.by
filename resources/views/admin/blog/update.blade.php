                ...
                <div class="col-md-9 offset-md-1">
                    <form action="{{ route('updateBlogPost') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <input value="{{ $blogPost->name }}" class="form-control" type="text" id="name" name="name" placeholder="Название категории">
                        </div>
                        <div class="form-group">
                            <label for="post_description">Описание</label>
                            <textarea class="form-control" type="text" id="post_description" name="description" placeholder="Описание">{{ $blogPost->description }}</textarea>
                        </div>
                        <div class="form-group">
                            <label for="post_content">Контент</label>
                            <textarea class="form-control" type="text" id="post_content" name="content" placeholder="Контент">{{ $blogPost->content }}</textarea>
                        </div>
                        <div class="form-group">
                            <label for="category">Категория</label>
                            <select class="form-control" name="category_id" id="category">
                                @foreach($blogCategories as $blogCategory)
                                    <option @if($blogPost->category_id == $blogCategory->id) selected @endif value="{{ $blogCategory->id }}">{{ $blogCategory->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                            <img id="preview_image" src="{{ $blogPost->image }}" alt="">
                            <div class="input__wrapper">
                                <label for=input__file"">Картинка поста</label>
                                <input value="{{ $blogPost->image }}" class="form-control input input__file" type="file" accept="image/*" id="input__file" name="image">
